...
DTableView::DTableView(QWidget* parent/* = nullptr*/)
{
    // Model
    tableModel = new DTableModel();
    this->setModel(tableModel);
    // Delegate
    DTableDelegate* tableDelegate = new DTableDelegate;
...
QList<QStringList> DTableView::Get_Value()
{
    return tableModel->ListPopBack();
}

void DTableView::Clear_All()
{
    tableModel->Clear_Table_Data();
}

void DTableView::View_Init()
{
    QStringList header;
    header << "" << "1" << "2";
